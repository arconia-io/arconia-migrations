...
When upgrading to Spring AI 1.0, refer to the https://docs.spring.io/spring-ai/reference/1.0/upgrade-notes.html[Upgrade Notes] for newly introduced APIs, deprecated APIs, and breaking changes.
...
* Rename Spring AI configuration properties that changed, and comment out those removed so you can address them manually.
* Remove Spring AI configuration properties that were dropped in 1.0, such as the per-provider `spring.ai.<provider>.<capability>.enabled` flags, and adjust changed values (for example, `spring.ai.chat.memory.repository.jdbc.initialize-schema` from `true`/`false` to `always`/`never`).
