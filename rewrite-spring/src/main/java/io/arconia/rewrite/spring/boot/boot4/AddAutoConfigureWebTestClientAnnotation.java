...
/**
 * In Spring Boot 3, many projects using Spring Web rely on WebTestClient
 * in @SpringBootTest integration tests, which is automatically configured
...
public class AddAutoConfigureWebTestClientAnnotation extends Recipe {
    ...
    public String getDescription() {
        return "Add missing `@AutoConfigureWebTestClient` annotation to classes annotated with " +
                "`@SpringBootTest` that use `WebTestClient`.";
    }
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return Preconditions.check(
                Preconditions.and(
                        new UsesType<>(FQN_SPRING_BOOT_TEST, false),
    ...
    private static class SpringBootTestVisitor extends JavaIsoVisitor<ExecutionContext> {
        ...
        private boolean isApplicableClass(J.ClassDeclaration classDeclaration) {
            ...
            // Not annotated with @SpringBootTest. No changes are needed.
            return false;
        }
    }
